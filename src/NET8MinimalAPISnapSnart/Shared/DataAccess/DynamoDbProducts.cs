...
namespace Shared.DataAccess
{
    public class DynamoDbProducts : ProductsDAO
    {
        ...
        private readonly ILogger<DynamoDbProducts> _logger;

        public DynamoDbProducts(AmazonDynamoDBClient dynamoDbClient, ILogger<DynamoDbProducts> logger)
        {
            _dynamoDbClient = dynamoDbClient;
            _logger = logger;
        }

        public async ValueTask Prime()
        {
            this._logger.LogInformation("Priming database connection");
            
            await this._dynamoDbClient.DescribeTableAsync(PRODUCT_TABLE_NAME);
            
            await this.GetAllProducts();
            await this.GetProduct("test");
            
            this._logger.LogInformation("Complete");
        }
